...
import { getNotEndedTasksWithPlannedExecutionTime, updateTask } from "../tasks/model";
...
export class Dispatcher {
    ...
    async onNewTask() {
        ...
        await this.plannTasksExecution();
        this.start();
    }
    ...
    private async plannTasksExecution() {
    ...
    private async executeTask(task: Task): Promise<void> {
        try {
            ...
            await new Promise((resolve, reject) => {
                exec(task.action, (error: Error | null, stdout: string, stderr: string) => {
                    if (error) {
                        reject(error);
                        return;
                    }
