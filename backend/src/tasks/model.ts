...
import { dateDbToISO, dateISOToDb } from "../utils";
...
export async function getTasks(): Promise<Task[]> {
  const rows =
    (await dbGetAll<Task[]>("SELECT * FROM tasks ORDER BY created_at DESC")) ||
    [];
  return rows.map(parseTaskFromDb);
}

export async function getNotEndedTasksWithPlannedExecutionTime(): Promise<
  Task[]
> {
  const rows =
    (await dbGetAll<Task[]>(`SELECT * FROM tasks 
        WHERE status = 'waiting' AND planned_execution_time IS NOT NULL ORDER BY created_at DESC`)) ||
    [];
  return rows.map(parseTaskFromDb);
}

export async function getAllWaitingTasks(): Promise<Task[]> {
  const rows =
    (await dbGetAll<Task[]>(`SELECT * FROM tasks 
...
export async function getWaitingTasksInDateRange(
  startDate: Date,
  endDate: Date
): Promise<Task[]> {
  const rows =
...
export async function addTask(task: TaskToAdd): Promise<Task> {
  const repeatingByWeekDay = task.repeating?.byWeekDay
    ? JSON.stringify(task.repeating.byWeekDay)
    : null;
  const repeatingByMonthDay = task.repeating?.byMonthDay
    ? JSON.stringify(task.repeating.byMonthDay)
    : null;
  const repeatingByMonth = task.repeating?.byMonth
    ? JSON.stringify(task.repeating.byMonth)
    : null;
  const repeatingByYearDay = task.repeating?.byYearDay
    ? JSON.stringify(task.repeating.byYearDay)
    : null;
  ...
  const result = await dbRun(
    ...
    [
      ...
      dateISOToDb(task.repeating?.until),
      dateISOToDb(task.repeating?.startDate),
      dateISOToDb(task.plannedExecutionTime),
      DEFAULT_TASK_STATUS,
    ]
  );
...
function parseTaskFromDb(row: any): Task {
  const repeating: TaskRepeating | undefined = row.repeating_frequency
    ? {
        ...
        byWeekDay: row.repeating_by_week_day
          ? JSON.parse(row.repeating_by_week_day)
          : undefined,
        byMonthDay: row.repeating_by_month_day
          ? JSON.parse(row.repeating_by_month_day)
          : undefined,
        byMonth: row.repeating_by_month
          ? JSON.parse(row.repeating_by_month)
          : undefined,
        byYearDay: row.repeating_by_year_day
          ? JSON.parse(row.repeating_by_year_day)
          : undefined,
  ...
  return {
    ...
    range: {
      start: dateDbToISO(row.range_start),
      end: dateDbToISO(row.range_end),
    },
    estimatedWorkingTime: row.estimated_working_time,
    estimatedWorkload: row.estimated_workload,
    repeating,
    plannedExecutionTime: dateDbToISO(row.planned_execution_time),
